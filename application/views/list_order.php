    ...
    <div class="clearfix">
        ...
        <?php if ($this->session->userdata['role_id'] == '2') : ?>
            <div class="float-right">
                <a href="<?php echo base_url('list_order/add_list_order') ?>" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;&nbsp;Tambah</a>
            </div>
        <?php endif; ?>
    </div>
    <hr>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <form method="get" action="<?= base_url('list_order') ?>" class="d-flex align-items-center mb-1">
                    ...
                    <tbody>
                        ...
                        foreach ($detail_order as $key => $value) { ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($value->apprWAHO == '1') : ?>
                                        <div class="bg-success rounded-pill text-center text-white">
                                            Approved
                                        </div>
                                    <?php elseif ($value->apprWAHO == '0') : ?>
                                        <div class="bg-danger rounded-pill text-center text-white">
                                            Rejected
                                        </div>
                                    <?php else : ?>
                                ...
                                <td>
                                    <?php if ($value->apprSH == '1' || $value->apprSH == '0') : ?>
                                        <a href="<?= base_url('list_order/detail_order/' . $value->id_order) ?>" class="btn btn-secondary">Detail</a>
                                    <?php else : ?>
                                        <a href="<?= base_url('list_order/edit_sparepart/' . $value->id_order) ?>" class="btn btn-secondary">Edit</a>
                                        <a href="<?= base_url('list_order/proses_delete/' . $value->id_order . '/' . $value->part_number) ?>" class="btn btn-danger">Delete</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php } ?>
