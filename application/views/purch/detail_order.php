                    ...
                    <div class="row">
                        <div class="col">
                            <div class="card shadow">
                                ...
                                <?php endif; ?>
                                <div class="card-body">
                                    <form action="<?= base_url('purch/list_order/approve') ?>" id="form-tambah" method="POST">
                                        <h5>Data Karyawan</h5>
                                        <hr>
                                        <div class="form-row">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($detail_order as $key => $detail) { ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <select name="part_number[]" id="part_number-<?= $i ?>" class="form-control" required readonly>
                                                            ...
                                                            <td>
                                                                <input type="text" name="wbs[]" value="<?= $detail->wbs == 0 || $detail->wbs == NULL ? '' : $detail->wbs ?>" id="wbs-<?= $i ?>" class="form-control" readonly required>
                                                            </td>
                                                            <td>
                                                                <select name="supplier[]" id="supplier-<?= $i ?>" class="form-control" required <?= $detail_order[0]->id_supplier == NULL ? '' : 'disabled' ?>>
                                                                    <option value="" selected="true" disabled="disabled">Pilih Spare Part</option>
                                                                    <?php foreach ($supplier as $supp) : ?>
                                                                        <option value="<?php echo $supp->id_supplier ?>" <?= $supp->id_supplier == $detail->id_supplier ? 'selected' : '' ?>><?= $supp->id_supplier ?> - <?= $supp->nama_supplier ?></option>
                                                                    <?php endforeach ?>
                                                                </select>
                                                            </td>
                                                        </tr>
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <td colspan="6" align="center">
                                                            <?php if ($detail_order[0]->id_supplier == NULL) : ?>
    ...
    <script>
        function Alert(approve = true) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `<?php echo base_url(); ?>purch/list_order/${approve ? 'approve' : 'reject/<?= $list_order[0]->id_order ?>'}`,
                        ...
                        success: () => {
                            Swal.fire({
                                ...
                                if (result.dismiss === Swal.DismissReason.timer) {
                                    location.href = '<?= base_url() ?>purch/list_order';
                                }
                            });
                        },
                        error: (error) => {
        ...
        $(document).ready(function() {
            $('#btn-approve').click(function() {
                var supplierElements = document.querySelectorAll('select[name="supplier[]"]');
                var isEmptyValueFound = false;
                supplierElements.forEach(function(element) {
